...
const Chatbot = () => {
  ...
  return (
    <div
      className={`fixed bottom-12 right-8 h-16 w-16 bg-blue-500 rounded-full finger ${
        !chat && "animate-bounce"
      }`}
    >
      <span
      ...
      {chat && (
        <div className="absolute bottom-full right-0 min-h-96 max-h-[35rem] w-[25rem] max-w-[100vw] bg-blue-500 rounded-md p-3 flex flex-col">
          ...
          <footer className="w-full bg-white rounded-b-md h-12 border-t border-zinc-500 flex items-center px-4">
            <input
              ...
              name="text"
              value={prompt}
              className="flex-1 p-1 border border-blue-500 outline-none rounded-s-md h-10"
              onChange={(e) => setPrompt(e.target.value)}
            />
            <button
              disabled={generating}
              className="text-white bg-blue-500 h-10 py-0 rounded-s-none"
              onClick={sendPrompt}
            >
              Send
            </button>
