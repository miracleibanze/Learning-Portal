...
const SendJoinRequest: FC<Props> = ({ course, close }) => {
  ...
  return (
    <main className="fixed inset-0 bg-black/90 z-[9999] flex justify-center items-start pt-20 overflow-auto">
      <div className="relative md:max-w-[30rem] w-full mx-4 animateIn bg-zinc-200 dark:bg-gray-900 dark:border border-white/50 rounded-md py-8 px-6 shadow-lg">
        <button
          onClick={close}
          ...
          {hasPrice && (
            <label className="flex items-start gap-2 cursor-pointer">
              ...
              <span>
                ...
                <strong className="text-sky-600">{course.price} RWF</strong>.
              </span>
            </label>
          )}
        </div>
        ...
        {hasPrice && agreedToPay && (
          <div className="mt-4 p-3 rounded-md bg-blue-100 dark:bg-blue-900/20 border border-blue-300 dark:border-blue-500 text-sm text-blue-800 dark:text-blue-300">
        ...
        {!isSubmitting && submitStatus === true && (
          <p className="body-2 leadin-tight py-2 mt-6 px-4 text-green-500">
