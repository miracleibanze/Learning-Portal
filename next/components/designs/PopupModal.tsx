...
const PopupModal: FC<PopupProps> = ({
  children,
  onClose,
  width,
  hFull,
  ...
  return (
    <main
      className={`fixed inset-0 bg-black/90 z-[9999] flex justify-center items-start pt-20 overflow-auto pb-8 ${
        className ? className : ""
      }`}
    >
      <div
        className={`relative ${
          ...
          hFull ? "flex-1 h-full flex flex-col" : ""
        }`}
      >
        {!noClose && (
          <button
