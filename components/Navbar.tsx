...
const Navbar: FC<{ user?: string }> = ({ user }) => {
  ...
  return (
    <nav className="w-full flex justify-between items-center sticky top-0 z-[9990] bg-blue-600 dark:bg-gray-900 border-b border-white/50 px-4 text-white">
      <Image
        src="/logo.png"
        alt="logo"
        width={160}
      ...
      <div className="flex items-center h-full">
        <ul
          className={`flex space-x-2 lg:justify-end max-lg:pt-24 h-full items-center max-lg:flex-col pt-2 lg:relative fixed max-lg:bg-blue-500 max-lg:text-white ${
            openNavigation
        ...
        {pathname === "/" && (
          <Link href={user ? "/dashboard" : "/login"}>
            <button className="h-10 lg:flex items-center ml-2 hidden bg-zinc-300 hover:scale-105 text-black hover:bg-blue-300/70 duration-300 rounded-md px-4 py-2">
              Dashboard
            </button>
          </Link>
        )}
